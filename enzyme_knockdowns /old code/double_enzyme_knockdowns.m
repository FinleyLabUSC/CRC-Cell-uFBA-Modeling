function matrix_of_fluxes = double_enzyme_knockdowns(j,cc)
...
% eg. if numpoints=4, 0% 33% 66% 100%. Since we want to just focus on
% enzyme knock*outs, set this to 2. 
...
for i=1:4    % i=1, WT(CRC-only media); i=2, KRAS(CRC-CAF media); i=3, WT(CRC-CAF media); i=4, KRAS(CRC-only media), this is the correct order
    ...
    for k=1:100
        ...
        if i==1
            if mean(WT_CRC(:,j)) > 0 && mean(WT_CRC(:,cc)) > 0
        ...
        if i==2
            if mean(KRAS_CCM(:,j)) > 0 && mean(KRAS_CCM(:,cc)) > 0
        ...
        if i==3
            if mean(WT_CCM(:,j)) > 0 && mean(WT_CCM(:,cc)) > 0
        ...
        if i==4
            if mean(KRAS_CRC(:,j)) > 0 && mean(KRAS_CRC(:,cc)) > 0
