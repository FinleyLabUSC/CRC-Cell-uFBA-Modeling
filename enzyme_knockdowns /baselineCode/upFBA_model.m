...
function upFBA_model(init_value_data, fold_change_kras_data, fold_change_wt_data, folder_path, conditions, title_name, rate_vals)
    ...
    'M_gln_L_e <o=> M_gln_L_i',...  % Gln transport
    ...
    '0.87841 M_3pg_c + 0.536023 M_pyr_c + 3.425741 M_oaa_i + 1.62 M_akg_i + 3.17174 M_accoa_m + 0.16035 M_r5p_ru5p_c + 100 M_atp_i + 100 M_h2o_c + 18 M_nadph_c  + 3.5 M_nad_i -> biomass + 3.17174 M_coa_m + 100 M_adp_i + 100 M_h_c + 100 M_pi_c + 18 M_nadp_c + 3.5 M_nadh_c',...
    'M_glu_L_m + M_atp_i + M_nh4_m -> M_gln_L_i + M_h_m + M_adp_i + M_pi_m'  % glutamine synthetase (GS)
    };

% Add in these extra reactions if desired
%     'M_pyr_c + M_co2_m + M_atp_i ->  M_oaa_i + M_adp_i + M_pi_m ',... % pyruvate carboxylase (PC)
%     'M_oaa_i +  M_glu_L_m -> M_asp_L_m + M_akg_i',... % aspartate aminotransferase (AA)
%     'M_asp_L_m + M_gln_L_i -> M_asn_L_m + M_glu_L_m' % asparagine synthetase (AS)
    ...
    SPECIES_BOUND_WT={
        ...
        "biomass[c]" rate_vals(7) rate_vals(7) ; % 10 biomass
    };
    ...
    Model_2 = changeRxnBounds(Model_2, 'Gln_tr', rate_vals(6), 'u'); 
    Model_2 = changeRxnBounds(Model_2, 'Gln_tr', rate_vals(6), 'l'); 
    KRAS_Model = struct;
